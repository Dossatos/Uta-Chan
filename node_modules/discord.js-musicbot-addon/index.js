...
exports.start = (client, options) => {
  class Music {
    constructor(client, options) {
      ...
      this.embedColor = (options && options.embedColor) || '0xffc0cb';
      this.thumbnailType = (options && options.thumbnailType) || "medium";
      ...
      this.enableQueueStat = Boolean((options && options.enableQueueStat) || true);
      ...
      this.clearOnLeave = (options && options.clearOnLeave) || true;
  ...
  musicbot.play = (msg, suffix) => {
    ...
    if (!suffix) return msg.channel.send(musicbot.note('fail', 'Master.. Can you be more specific?'));
    ...
    msg.channel.send(musicbot.note('search', `Looking for: \"\`${searchstring}\`\"`))
      .then(response => {
          ...
          ypi.playlistInfo(musicbot.youtubeKey, playid, function(playlistItems) {
            ...
            newItems.forEach(video => {
              ...
              if (ran == newItems.length) {
                if (index = 0) msg.channel.send(musicbot.note('fail', `I coudln't get any songs from that playlist.. Sorry master...`))
  ...
  musicbot.search = (msg, suffix) => {
    musicbot.dInvoker(msg);
    // Make sure the user is in a voice channel.
    if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Master.. You're not in a voice channel~`));
    ...
    msg.channel.send(musicbot.note('search', `Searching: \`${searchstring}\``))
      .then(response => {
        musicbot.searcher.search(searchstring, {
          ...
          .then(searchResult => {
            if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Failed to get search results.. Sorry master...'));
  ...
  musicbot.np = (msg, suffix) => {
    ...
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    const queue = musicbot.getQueue(msg.guild.id, true);
    if (voiceConnection === null && queue.length > 0) return msg.channel.send(musicbot.note('fail', 'No music is being played, how boring... Why don\'t you continue playing current queue?'));
    else if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No music is being played, how boring...'));
    const dispatcher = voiceConnection.player.dispatcher;
  ...
  musicbot.pause = (msg, suffix) => {
    ...
    if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', 'Only my real master may do that!'));
  ...
  musicbot.leave = (msg, suffix) => {
    ...
    if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Master can you let me in?'));
      ...
      voiceConnection.player.dispatcher.end();
      voiceConnection.disconnect();
      msg.channel.send(musicbot.note('note', 'Bye~! Master! :two_hearts:'));
    ...
    if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanJoin === true) {
      if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
        msg.member.voiceChannel.join().then((connection) => {
          msg.channel.send(musicbot.note("note", "Uta Chan already came to your channel nano desu! :two_hearts: "));
        }).catch((error) => {
            console.error(error);
            msg.channel.send(musicbot.note("fail", "Sorry! My master maybe I'm sick..."));
          });
      } else if (!msg.member.voiceChannel) {
        return msg.channel.send(musicbot.note("fail", "I can't find you anywhere my master..?"));
      } else if (msg.member.voiceChannel && !msg.member.voiceChannel.joinable) {
        return msg.channel.send(musicbot.note("fail", "Somehow I can't having fun with you, my master..."));
      };
    } else {
      const chance = Math.floor((Math.random() * 100) + 1);
      if (chance <= 10) return msg.channel.send(musicbot.note('fail', `I'm afraid I can't let you do that, ${msg.author.username}.`))
  ...
  musicbot.clearqueue = (msg, suffix) => {
    musicbot.dInvoker(msg)
    if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', 'No server queue? How boring...'));
    ...
    if (musicbot.isAdmin(msg.member)) {
      musicbot.emptyQueue(msg.guild.id).then(() => {
        ...
        msg.channel.send(musicbot.note('note', 'Queue cleared! :two_hearts:'));
      }).catch((res) => {
  ...
  musicbot.resume = (msg, suffix) => {
    ...
    // Get the voice connection.
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No music is being played, how boring...'));
  ...
  musicbot.volume = (msg, suffix) => {
    ...
    if (suffix > 200 || suffix <= 0) return msg.channel.send(musicbot.note('fail', 'Your volume! Master!, must be within 1 - 200 or else your ear will explode!!!'));
  ...
  musicbot.set = (msg, suffix, args) => {
    if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note('fail', `Only my real master may do that!`));
    const valid = ["prefix"];
    ...
    if (args[0] && args[0] == "prefix") {
      const newPrefix = args.slice(1, args.length)
        .join(" ");
      if (!newPrefix || newPrefix.legnth < 1) return msg.channel.send(musicbot.note('fail', 'Please specify a prefix, my master.'));
  ...
  musicbot.executeQueue = (msg, queue) => {
    ...
    musicbot.isQueueEmpty(queue).then(res => {
      if (res == true) {
        msg.channel.send(musicbot.note('note', 'Playback finished!? Aw.. I want more...'));
    ...
    new Promise((resolve, reject) => {
        ...
        if (voiceConnection === null) {
          ...
          if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
            ...
            msg.channel.send(musicbot.note('fail', 'I have no right to get in your room, please give permission, master.'))
            reject();
          } else {
            ...
            setTimeout(() => {
              ...
              if (nqueue.loop == "single") {
                ...
                if (queue.songs.length > 0) { // Remove the song from the queue.
                  ...
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', 'Playback finished!? Aw.. I want more...'));
  ...
  musicbot.note = (type, text) => {
    if (type === 'wrap') {
      ...
      return ':minidisc:  | ' + text.replace(/`/g, '`' + String.fromCharCode(8203));
    } else if (type === 'search') {
      return ':hourglass_flowing_sand:  | ' + text.replace(/`/g, '`' + String.fromCharCode(8203));
    } else if (type === 'fail') {
      return ':pushpin:  | ' + text.replace(/`/g, '`' + String.fromCharCode(8203));
    } else if (type === 'font') {
